...
export class JobLogsRepository {
  ...
  finished(id: bigint, counts?: { tasksSynced?: number; timeEntriesSynced?: number }, status = 'completed', message?: string) {
    return this.prisma.syncJobLog.update({ where: { id }, data: { status, finishedAt: new Date(), ...counts, ...(message ? { errorMessage: message } : {}) } });
  }
