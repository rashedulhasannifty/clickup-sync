...
describe("CustomFieldExtractor", () => {
    ...
    const task = taskWith([
      {
        ...
        value: 0,
        type_config: {
          options: [{ orderindex: 0, name: "Call A tradie Pty " }],
        },
      },
    ]);
  ...
  it("maps a whitespace-only string field to null rather than an empty string", () => {
    const task = taskWith([
      { name: "Department", type: "short_text", value: "   " },
    ]);
  ...
  describe("sub-project", () => {
      ...
      const task = taskWith([
        {
          ...
          type_config: {
            options: [
              { id: "uuid-a", label: "Mobile App" },
              { id: "uuid-b", label: "Website" },
            ],
          },
        },
      ]);
      ...
      const task = taskWith([
        {
          ...
          type: "labels",
          value: ["a"],
          type_config: { options: [{ id: "a", name: "Portal" }] },
        },
      ]);
      ...
      const task = taskWith([
        {
          ...
          type: "drop_down",
          value: 2,
          type_config: { options: [{ orderindex: 2, name: "Portal" }] },
        },
      ]);
      expect(extractor.extract(task).subProjects).toEqual(["Portal"]);
    });
      ...
      const task = taskWith([
        {
          ...
          value: "opt-1",
          type_config: {
            options: [{ id: "opt-1", orderindex: 0, name: "Portal" }],
          },
        },
      ]);
      expect(extractor.extract(task).subProjects).toEqual(["Portal"]);
    ...
    it("stores a text variant as a single trimmed value", () => {
      const task = taskWith([
        { name: "Sub-Project", type: "short_text", value: " Portal " },
      ]);
      expect(extractor.extract(task).subProjects).toEqual(["Portal"]);
    });

    it.each(["Sub-Project", "Sub Project", "subproject", "SUB_PROJECT"])(
    ...
    it('does not treat a plain "Project" field as a sub-project', () => {
      const task = taskWith([
        { name: "Project", type: "short_text", value: "Portal" },
      ]);
      expect(extractor.extract(task).subProjects).toEqual([]);
    });
    ...
    it("returns an empty list when the field is absent or unset", () => {
      ...
      expect(
        extractor.extract(
          taskWith([{ name: "Sub-Project", type: "labels", value: [] }]),
        ).subProjects,
      ).toEqual([]);
      expect(
        extractor.extract(taskWith([{ name: "Sub-Project", type: "labels" }]))
          .subProjects,
      ).toEqual([]);
    });
  });
