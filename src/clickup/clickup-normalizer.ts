...
export interface NormalizedTask {
  ...
  watchersNames: string | null;
  watchersEmails: string | null;
  ...
  normalizeTask(t: ClickUpTask): NormalizedTask {
    ...
    return {
      ...
      timeEstimate: t.time_estimate
        ? BigInt(Math.round(toNumberOrZero(t.time_estimate)))
        : null,
      timeSpent: t.time_spent
        ? BigInt(Math.round(toNumberOrZero(t.time_spent)))
        : null,
      spaceId: toStringOrNull(t.space?.id),
      spaceName: toStringOrNull(t.space?.name),
      folderId: toStringOrNull(t.folder?.id),
      folderName: toStringOrNull(t.folder?.name),
      listId: toStringOrNull(t.list?.id),
      listName: toStringOrNull(t.list?.name),
      assigneesNames: joinNames(t.assignees || [], "username"),
      assigneesEmails: joinNames(t.assignees || [], "email"),
      watchersNames: joinNames(t.watchers || [], "username"),
      watchersEmails: joinNames(t.watchers || [], "email"),
      creatorId: toStringOrNull(t.creator?.id),
      creatorName: toStringOrNull(t.creator?.username),
      executiveName: cf.executiveName,
      department: cf.department,
      client: cf.client,
      clientOptionId: cf.clientOptionId,
      subProjects: cf.subProjects,
      cost: cf.cost,
      estimation: cf.estimation,
      sprintName: cf.sprintName,
      sprintPoints: cf.sprintPoints,
      tags: joinNames(t.tags || [], "name"),
      customTags: joinNames(t.tags || [], "name"),
      raw: t,
    };
  }
  ...
  normalizeTimeEntry(entry: ClickUpTimeEntry): NormalizedTimeEntry {
    ...
    return {
      ...
      durationHours: entry.duration
        ? Math.max(0, toNumberOrZero(entry.duration) / 3600000)
        : 0,
