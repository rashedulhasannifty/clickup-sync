...
export class TaskReconciliationService {
  ...
  async reconcileTask(taskId: string, startMs: number, endMs: number): Promise<ReconcileTaskResult> {
    let task: unknown;
    try {
      task = await this.clickup.getTask(taskId);
    } catch (err: any) {
      ...
      if (err?.response?.status === 404) {
        ...
        await this.tasks.softDeleteTask(taskId);
    ...
    await this.tasks.syncTasks([task]);
    const timeEntriesSynced = await this.timeEntries.syncTaskTimeEntries(taskId, undefined, startMs, endMs);
