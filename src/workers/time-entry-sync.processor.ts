...
export class TimeEntrySyncProcessor extends WorkerHost {
  ...
  async process(job: Job<{ taskId: string; assigneeId?: string; startDate?: number; endDate?: number }>) {
    ...
    try {
      const result = await this.timeEntries.syncTaskTimeEntries(job.data.taskId, job.data.assigneeId, job.data.startDate, job.data.endDate);
