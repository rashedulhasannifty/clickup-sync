...
- **Scope column.** `clickup_tasks.scope_client_option_id` (nullable, indexed) is the row's own `client_option_id` when set, else inherited one level from its parent task (subtasks of subtasks inherit through their direct parent). It is maintained by every task write path and by the one-off `src/scripts/backfill-client-option-ids.ts` for rows synced before migration `0023_team_scoped_access`. A task with no resolvable client (`scope_client_option_id IS NULL`) is visible to OWNER/ADMIN only — never to a scoped MEMBER — under both `visibleClientIds` and `leadClientIds`.
...
- **The flag.** `preferences.access.teamScopingEnabled` (default `false`), read via `SettingsService.isTeamScopingEnabled()`. Off, `resolveScope` returns `{ kind: 'unrestricted', canEdit: false }` for a MEMBER — reads everything, writes nothing, byte-for-byte the pre-Teams behaviour. On, a MEMBER is scoped to their teams' clients; a LEAD of a team additionally sees cost for that team's clients, can toggle chargeability there, and can open a teammate's timesheet. Toggled Owner-only from Settings (`PATCH /admin/settings`, already audited via `AdminController`'s class-level `AuditLogInterceptor`); takes effect on the next request, no redeploy. See "Team-scoped access rollout" in `docs/OPERATIONS.md`.
