...
describe("CustomFieldExtractor", () => {
      ...
      custom_fields: [
        {
          ...
          type: "drop_down",
          value: 1,
          type_config: { options: [{ orderindex: 1, name: "Acme" }] },
        },
  ...
  it('extracts cost and estimation, including the "estimate" alias', () => {
    const cost = extractor.extract({
      id: "1",
      custom_fields: [{ name: "Cost", value: "125.5" }],
    } as any);
    expect(cost.cost).toBe(125.5);

    const estimation = extractor.extract({
      id: "1",
      custom_fields: [{ name: "Estimation", value: 8 }],
    } as any);
    expect(estimation.estimation).toBe(8);

    const estimateAlias = extractor.extract({
      id: "1",
      custom_fields: [{ name: "Time Estimate", value: 4 }],
    } as any);
  ...
  describe("sprint name vs sprint points disambiguation", () => {
    ...
    it('does NOT read "Sprint Points" into sprintName (the !point guard)', () => {
      const result = extractor.extract({
        id: "1",
        custom_fields: [{ name: "Sprint Points", value: 5 }],
      } as any);
  ...
  describe("sprint points root-level fallback", () => {
    it("falls back to root-level points when no custom field is present", () => {
      expect(
        extractor.extract({ id: "1", points: 3 } as any).sprintPoints,
      ).toBe(3);
    });

    it("falls back to root-level story_points when points is absent", () => {
      expect(
        extractor.extract({ id: "1", story_points: 7 } as any).sprintPoints,
      ).toBe(7);
    });

    it("prefers points over story_points at root level", () => {
      expect(
        extractor.extract({ id: "1", points: 2, story_points: 9 } as any)
          .sprintPoints,
      ).toBe(2);
    });
    ...
    it("truncates fractional sprint points to an integer", () => {
      expect(
        extractor.extract({ id: "1", points: 3.9 } as any).sprintPoints,
      ).toBe(3);
    });
    ...
    it("clamps an out-of-int4-range value to 0 so it cannot overflow the column", () => {
      ...
      const fromCustomField = extractor.extract({
        id: "1",
        custom_fields: [{ name: "Contact Point", value: 10001784894520 }],
      } as any);
      expect(fromCustomField.sprintPoints).toBe(0);

      // Same guard on the root-level source.
      expect(
        extractor.extract({ id: "1", points: 10001784894520 } as any)
          .sprintPoints,
      ).toBe(0);
      // Negative is nonsensical for points → 0.
      expect(
        extractor.extract({ id: "1", points: -5 } as any).sprintPoints,
      ).toBe(0);
      // A large-but-valid int4 value still passes through.
      expect(
        extractor.extract({ id: "1", points: 2000000000 } as any).sprintPoints,
      ).toBe(2000000000);
    });
  });
  ...
  describe("client dropdown resolution", () => {
    ...
    it("returns null when type_config / options are missing", () => {
      const result = extractor.extract({
        id: "1",
        custom_fields: [{ name: "Client", type: "drop_down", value: 0 }],
      } as any);
      expect(result.client).toBeNull();
    });
