...
import { CalendarClock, Download, AlertTriangle, Clock3 } from 'lucide-react';
...
.ts-task { transition: background 100ms; }
        ...
        <>
          ...
          {loading ? (
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
              ...
              {(sheet?.days ?? []).map((day) => (
                <DayGroup key={day.date} day={day} showCost={showCost} maxHours={maxDayHours} renderMoney={money} />
              ))}
...
function DayGroup({
  ...
  return (
    <div className="ts-day" style={{ borderTop: '1px solid var(--border-soft)' }}>
      ...
      {/* Task rows */}
      {day.tasks.map((t) => (
        <div key={t.taskId} className="ts-task" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '7px 16px' }}>
          <span style={{ width: DATE_COL }} />
          <span style={{
            flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 8,
            paddingLeft: 14, borderLeft: '2px solid var(--border)',
          }}>
            <span title={t.taskName ?? t.taskId} style={{
